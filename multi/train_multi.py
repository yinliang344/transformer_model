from multi.transformer_HAN_multi import HAN_model
...
valid = validation()
DP = DataProcess()
train_x_all, train_y_input_float, train_y_input_int, train_y_output_all, test_x_all, test_y_input_float, test_y_input_int, test_y_output_all= DP.read_file_multi(r'./data_multi')
print('1、load data完成')
Model = HAN_model()
print('2、构造模型完成')
...
with tf.Session() as sess:
    ...
    writer_train = tf.summary.FileWriter("./multi/logs_multi/train", sess.graph)
    writer_test = tf.summary.FileWriter("./multi/logs_multi/test")
    sess.run(init_op)
        ...
        for i in trange(epoch_number,desc='epoch-'+str(k+1)+'/'+str(cf.epoch_size)+':'):
            ...
            if (j)%100==0:
                ...
                if smoothing_T>=smoothing_xq:
                    ...
                    saver.save(sess, save_path='./multi/ckpt_multi/model.ckpt', global_step=j)