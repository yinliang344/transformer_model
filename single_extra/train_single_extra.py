from single_extra.tranformer_HAN_single_extra import HAN_model
...
with tf.Session() as sess:
    ...
    writer_train = tf.summary.FileWriter("single_extra/logs_single_extra/train", sess.graph)
    writer_test = tf.summary.FileWriter("single_extra/logs_single_extra/test")
    sess.run(init_op)
    ...
    for k in range(cf.epoch_size):
        for i in trange(epoch_number,desc='epoch-'+str(k+1)+'/'+str(cf.epoch_size)+':'):
            ...
            feed_dic_train = {Model.input_data: np.array(batch_data_train),
                              ...
                              Model.input_label_xq:batch_label_train,
                              Model.keep_rate: 0.7,
            ...
            if (j)%100==0:
                saver.save(sess, save_path='single_extra/ckpt_single_extra/model.ckpt', global_step=j)
                ...
                rs_test = sess.run(merged,feed_dict=feed_dic_test)
