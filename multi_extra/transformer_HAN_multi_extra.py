...
class HAN_model():
    def __init__(self):
        ...
        with tf.name_scope('input'):
            self.input_data = tf.placeholder(
                dtype=tf.int32, shape=(
                    None, self.truncature_len), name='input_data')
            self.input_shuxing_float = tf.placeholder(dtype=tf.float32, shape=(
                None, self.shuxing_number_float), name='input_shuxing_float')
            self.input_shuxing_int = tf.placeholder(dtype=tf.int32, shape=(
                None, self.shuxing_number_int), name='input_shuxing_int')
            self.input_label = tf.placeholder(
                dtype=tf.int64, shape=(
                    None, 3), name='input_label_xq')
            self.keep_rate = tf.placeholder(
                dtype=tf.float32, shape=(None), name='keep')
            self.is_trainning = tf.placeholder(
        ...
        with tf.name_scope('embedding'):
            ...
            data_embedding = tf.nn.embedding_lookup(
                embedding_table, self.input_data)
            data_embedding = MHA.layer_norm(
                data_embedding, scope='embedding_ln_norm')
            self.xb_embedding = self.embedding(name='xq_embedding', shape=(
                xb_number, 32), input=tf.slice(self.input_shuxing_int, [0, 0], [-1, 1]))
            # print(self.xb_embedding)
            self.jycd_embedding = self.embedding(name='jycd_embedding', shape=(
                jycd_number, 32), input=tf.slice(self.input_shuxing_int, [0, 1], [-1, 1]))
            # print(self.jycd_embedding)
            self.qkzm_embedding = self.embedding(name='qkzm_embedding', shape=(
                zm_number, 64), input=tf.slice(self.input_shuxing_int, [0, 2], [-1, 1]))
            self.jl_embedding = self.embedding(name='jl_embedding', shape=(
                jl_number, 32), input=tf.slice(self.input_shuxing_int, [0, 3], [-1, 1]))
            self.je_embedding = self.embedding(name='je_embedding', shape=(
                je_number, 32), input=tf.slice(self.input_shuxing_int, [0, 4], [-1, 1]))
            self.qj_embedding = self.embedding(name='qj_embedding', shape=(
                qj_number, 32), input=tf.slice(self.input_shuxing_int, [0, 5], [-1, 1]))
            self.hg_embedding = self.embedding(name='hg_embedding', shape=(
                hg_number, 32), input=tf.slice(self.input_shuxing_int, [0, 6], [-1, 1]))
            self.rztd_embedding = self.embedding(name='rztd_embedding', shape=(
                rztd_number, 32), input=tf.slice(self.input_shuxing_int, [0, 7], [-1, 1]))

        with tf.name_scope('encoder'):
            self.data_cov_1 = self.conv1D(
                inputs=data_embedding,
                kernel_shape=(
                    3,
                    self.embedding_size,
                    self.embedding_size),
            ...
            self.data_cov_2 = self.conv1D(
                inputs=self.data_cov_1,
                kernel_shape=(
                    3,
                    self.embedding_size,
                    self.embedding_size),
            ...
            self.data_word_lavel = tf.reshape(
                self.data_cov_2, shape=(-1, self.len_word, self.embedding_size))
            ...
            # shape=(128, 25, 600)
            self.data_sq_lavel = tf.reshape(
                self.word_atten, shape=(-1, self.len_sq, np.shape(self.word_atten)[-1]))
            # shape=(128, 25, 600)
        ...
        with tf.name_scope('sx_encoder'):
            self.sx_float = self.sx_dense_layer(
                name='sx_float', inputs=self.input_shuxing_float, output_size=32)
            self.xb_int = self.sx_dense_layer(
                name='xb_int', inputs=self.xb_embedding, output_size=32)
            self.jycd_int = self.sx_dense_layer(
                name='jycd_int', inputs=self.jycd_embedding, output_size=32)
            self.qkzm_int = self.sx_dense_layer(
                name='qkzm_int', inputs=self.qkzm_embedding, output_size=64)
            # self.xflb_int = self.sx_dense_layer(name='xflb_int',inputs=self.xflb_embedding,output_size=32)
            self.jl_int = self.sx_dense_layer(
                name='jl_int', inputs=self.jl_embedding, output_size=32)
            self.je_int = self.sx_dense_layer(
                name='je_int', inputs=self.je_embedding, output_size=32)
            self.qj_int = self.sx_dense_layer(
                name='qj_int', inputs=self.qj_embedding, output_size=32)
            self.hg_int = self.sx_dense_layer(
                name='hg_int', inputs=self.hg_embedding, output_size=32)
            self.rztd_int = self.sx_dense_layer(
                name='rztd_int', inputs=self.rztd_embedding, output_size=32)

        with tf.name_scope('classifier'):
            self.all_weight_ft = tf.concat([self.ft_atten,
                                            self.sx_float,
                                            self.xb_int,
                                            self.jycd_int,
                                            self.qkzm_int,
                                            ...
                                            self.rztd_int],
                                           axis=-1)
            self.all_weight_xq = tf.concat([self.xq_atten,
                                            self.sx_float,
                                            self.xb_int,
                                            self.jycd_int,
                                            self.qkzm_int,
                                            ...
                                            self.rztd_int],
                                           axis=-1)
            self.all_weight_zm = tf.concat([self.zm_atten,
                                            self.sx_float,
                                            self.xb_int,
                                            self.jycd_int,
                                            self.qkzm_int,
            ...
            self.zm_dense_3 = self.fully_conacation(name='zm_3',
                                                    ...
                                                    activation='relu')
            # 刑期分类器
            self.weight_xq = tf.concat(
                [self.all_weight_xq, self.ft_dense_2, self.zm_dense_2], axis=-1)
            self.xq_dense_1 = self.fully_conacation(name='xq_1',
                                                    input=self.weight_xq,
                                                    haddin_size=1024,
                                                    training=self.is_trainning,
                                                    keep_rate=self.keep_rate,
                                                    activation='relu')
        ...
        with tf.name_scope('classifier_loss'):
            # 将输入切片，分为法条，刑期，罪名
            self.label_ft = tf.reduce_sum(
                tf.slice(self.input_label, [0, 0], [-1, 1], name='rule_slice'), axis=-1)
            self.label_xq = tf.reduce_sum(
                tf.slice(self.input_label, [0, 1], [-1, 1], name='zm_slice'), axis=-1)
            self.label_zm = tf.reduce_sum(
                tf.slice(self.input_label, [0, 2], [-1, 1], name='xq_slice'), axis=-1)

            # 计算法条损失
            self.ft_loss = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(
                    labels=self.label_ft, logits=self.ft_dense_3), axis=-1)
            tf.add_to_collection('loss', self.ft_loss)
            tf.summary.scalar('ft_loss', self.ft_loss)
            # print(self.ft_loss)

            # 计算刑期损失
            self.xq_loss = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(
                    labels=self.label_xq, logits=self.xq_dense_3), axis=-1)
            tf.add_to_collection('loss', self.xq_loss)
            tf.summary.scalar('xq_loss', self.xq_loss)
            # print(self.xq_loss)

            # 计算罪名损失
            self.zm_loss = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(
                    labels=self.label_zm, logits=self.zm_dense_3), axis=-1)
    ...
    def Dynamic_LSTM(
            ...
            training,
            name,
            init_hadden_state_fw=None,
            init_hadden_state_bw=None):
        with tf.variable_scope("lst_" + str(name) + "_1"):
            cell_f_1 = tf.nn.rnn_cell.BasicLSTMCell(hidden_size)
            cell_f_1 = tf.nn.rnn_cell.DropoutWrapper(
                cell_f_1, output_keep_prob=keep_rate)
            cell_b_1 = tf.nn.rnn_cell.BasicLSTMCell(hidden_size)
            cell_b_1 = tf.nn.rnn_cell.DropoutWrapper(
                cell_b_1, output_keep_prob=keep_rate)
    ...
    def attention(
        ...
        with tf.variable_scope(name + '_att'):
            ...
            u_atten = tf.get_variable(name=name + '_att_vocter',
                                      shape=(1,
                                             np.shape(inputs)[-1]),
                                      dtype=tf.float32,
                                      initializer=initializer)
            att_weght = tf.reduce_sum(
                tf.multiply(
                    transfor_data,
                    u_atten),
                keep_dims=True,
        ...
        with tf.name_scope('conv1d_' + kernel_name):
            ...
            if dropuot_rate is not None:
                conv_output = tf.nn.dropout(
                    conv_output, keep_prob=dropuot_rate)
            return conv_output
